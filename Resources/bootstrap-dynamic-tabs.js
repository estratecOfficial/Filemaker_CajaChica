			...
			function DynamicTabs() {
				...
				$tabs.each(function (i) {
					$(this)
						...
						console.log("valor del div es ="+i);
				});
				...
				var onDropDow = function () {
					...
					$horizontalContainer.children('li').each( function (index, element) {
						if ($(element).children('a').css("display") !== 'none') {
							...
							/*console.log("el id es:"+$(element).attr("id"));*/
							/*var idConseguido = $(element).children('a').attr("id");*/
							/*$(htab).append('<a class = "'+idConseguido+'" href="#">'+$(element).children('a').html()+'</a>');
*/
							$(htab).append('<a href="#">'+$(element).children('a').html()+'</a>');

							/*console.log("el nene es:"+$(element).children('a').html());*/
							console.log("sera el elemento:"+$(element).children('a').attr("id"));
							...
							$(htab).on("click", function (e) {
								...
								updateTabs();
								console.log("el tab-id es:"+ $(this).attr("tab-id"));
								console.log("el html es:"+ $(element).children('a').html());
								/*ejecutar($(element).children('a').html());*/
								ejecutar($(element).children('a').attr("id"));
							});
			...
			function ejecutar(valor){
				...
				if(valor=="menu01"){
					console.log("ejecutando: estac "+valor);
					RecargarTablaEstacionamiento();
				}else if(valor=="menu02"){
					console.log("ejecutando: Viaje "+valor);
					RecargarTablaViajes();
				}else if(valor=="menu03"){
					console.log("ejecutando:otros "+valor);
					RecargarTablaVales();
				}else if(valor=="menu04"){
					console.log("ejecutando:"+valor);
					console.log("si llego");
					RecargarTablaValesProvisionales();
				}else if(valor=="menu05"){
					console.log("ejecutando: caseta:"+valor);
					RecargarTablaCasetas();
				}else{
					console.log("prueba de nuez");
				}
			}
